...
let rev_ind_redirs redirs =
  List.filter (flip List.mem_assq redirs <. fst) @@ rev_redirs redirs
 ...
 *)
let rec rdfs addr2indeg rev_ind_redirs node_ref = 
  let indeg = List.assoc node_ref addr2indeg in (* 現ノードの間接参照からの参照を考慮しない被参照数 *)
  maybe ([(node_ref, indeg)], indeg) (* 逆辺で辿れるノードが存在しないなら現ノードの被参照数に関する情報のみを返す *)
    @@ let+ parents = List.assq_opt node_ref rev_ind_redirs in
       (* 逆辺で辿れる間接ノードを深さ優先探索する *)
       let addr2indeg, parent_indegs =
	 List.split 
	 @@ List.map (rdfs addr2indeg rev_ind_redirs) parents in
       let indeg = List.fold_left (+) 0 parent_indegs + indeg in
       ((node_ref, indeg)::List.concat addr2indeg, indeg)
 ...
 *) 
let ref_count_redir addr2indeg redirs rev_ind_redirs visited node_ref =
  if List.memq node_ref visited then Some [] (* もうすでに訪れたノードなら何もしない *)
	...
	@@ List.assq node_ref redirs in 
      ...
      Some (fst @@ rdfs addr2indeg rev_ind_redirs node_ref)

let ref_count_redirs addr2indeg redirs rev_ind_redirs =
  ...
  let rec check_all new_addr2indeg = function
    ...
    | node_ref::rest ->
       ...
       let* new_addr2indeg' = 
	 ref_count_redir addr2indeg redirs rev_ind_redirs visited node_ref in
...
let check_redirs (redirs, free_indeg_diffs) env =
  ...
  let rev_ind_redirs = rev_ind_redirs redirs in

  match ref_count_redirs addr2indegs redirs rev_ind_redirs with
  | None -> (* ダメだったので巻き戻し *)
  ...
  | Some addr2indegs ->
     let update_indeg (node_ref, indeg) =
       update_ref (first @@ const indeg) node_ref
     in
